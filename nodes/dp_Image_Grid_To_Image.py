...
class DP_Image_Grid_To_Image:
    ...
    def stitch_image(
        ...
        for idx, slice_img in enumerate(slice_list):
            ...
            if stroke_size > 0:
                color = torch.tensor(
                    self.get_stroke_color(stroke_color),
                    device=slice_img.device,
                    dtype=slice_img.dtype,
                )

                # Create stroke mask
                stroke_mask = torch.ones_like(slice_img)
                stroke_mask[
                    :, stroke_size:-stroke_size, stroke_size:-stroke_size, :
                ] = 0

                # Apply stroke color
                slice_img = (
                    slice_img * (1 - stroke_mask) + color.view(1, 1, 1, 3) * stroke_mask
                )
