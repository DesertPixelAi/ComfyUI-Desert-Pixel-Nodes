...
class DP_Prompt_Styler:
            ...
            if mode == "Randomize_All":
                ...
                for style, category in [
                    ...
                    if style == "randomize":
                        ...
                        options = [opt for opt in self.CATEGORIES[category] if opt not in ["none", "randomize"]]
                        if options:
    ...
    def IS_CHANGED(cls, **kwargs):
        ...
        if (kwargs.get('mode') == "Randomize_All" or
            any(kwargs.get(key) == "randomize" for key in [
                "DepthStyle", "cameraAngles", "colorTheme", "FaceMood",
                "timeOfDay", "atmosphere", "lighting", "filter_effect", "CameraType"
            ])):
