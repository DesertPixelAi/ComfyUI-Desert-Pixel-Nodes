...
class DP_Load_Image_Effects:
    ...
    def load_image_and_process(self, image, effect_strength, effect_A, effect_B,
                             effect_C, effect_D, resize_image, width, height):
        prompt_text = ""
        negative_text = ""
        results = []
    ...
    def IS_CHANGED(s, image, effect_strength=1.0, effect_A="original", effect_B="original",
                  effect_C="original", effect_D="original", resize_image=True, width=1024, height=1024):
        image_path = folder_paths.get_annotated_filepath(image)
        return f"{image_path}_{effect_strength}_{effect_A}_{effect_B}_{effect_C}_{effect_D}_{resize_image}_{width}_{height}"

    @classmethod
    def VALIDATE_INPUTS(s, image, effect_strength, effect_A, effect_B,
                       effect_C, effect_D, width, height):
        if not folder_paths.exists_annotated_filepath(image):
            return "Invalid image file: {}".format(image)
        return True

    RETURN_TYPES = ("IMAGE", "IMAGE", "IMAGE", "IMAGE", "STRING", "STRING", "STRING")
    RETURN_NAMES = ("image_A", "image_B", "image_C", "image_D", 
                   "filename", "dp_prompt", "dp_negative_or_other")
