...
class DP_Video_Flicker:
    ...
    def INPUT_TYPES(cls):
        return {
            "required": {
                ...
                "Flicker_1_size": (
                    "INT",
                    {"default": 3, "min": 3, "max": 10, "step": 1},
                ),
                "Flicker_1_speed": (
                    "INT",
                    {"default": 1, "min": 1, "max": 3, "step": 1},
                ),
                "Flicker_1_color": ("STRING", {"default": "#000000"}),
            },
            "optional": {
                "Flicker_2_key_frame": (
                    "INT",
                ...
                "Flicker_2_color": ("STRING", {"default": "#000000"}),
                "Flicker_3_key_frame": (
                    "INT",
                    {"default": 0, "min": 0, "max": 10000, "step": 1},
                ),
                ...
                "Flicker_3_color": ("STRING", {"default": "#000000"}),
            },
        }
    ...
    def execute(
        ...
        Flicker_1_speed,
        Flicker_1_color,
        Flicker_2_key_frame=0,
        Flicker_2_size=3,
        Flicker_2_speed=1,
        Flicker_2_color="#000000",
        Flicker_3_key_frame=0,
        Flicker_3_size=3,
        Flicker_3_speed=1,
        Flicker_3_color="#000000",
    ):
        ...
        if Flicker_1_key_frame > 0:
            flicker1_end = Flicker_1_key_frame + (
                Flicker_1_size * (Flicker_1_speed + 1)
            )
        ...
        if Flicker_2_key_frame > 0:
            flicker2_end = Flicker_2_key_frame + (
                Flicker_2_size * (Flicker_2_speed + 1)
            )
        ...
        if Flicker_3_key_frame > 0:
            flicker3_end = Flicker_3_key_frame + (
                Flicker_3_size * (Flicker_3_speed + 1)
            )
            should_add = True
        ...
        while current_frame < frames_to_export:
            ...
            for settings in flicker_settings:
                ...
                if (
                    settings["keyframe"]
                    <= current_frame
                    < settings["keyframe"] + sequence_length
                ):
                    frame_offset = current_frame - settings["keyframe"]
                    if (
                        frame_offset % (settings["speed"] + 1) == 0
                        and frame_offset < sequence_length
                    ):
                        flicker_frame = torch.stack(
                            [
                                torch.full_like(
                                    Load_video_frames[0, :, :, c],
                                    fill_value=settings["color"][c],
                                )
                                for c in range(3)
                            ],
                            dim=-1,
