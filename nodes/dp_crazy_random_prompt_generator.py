...
class DP_Random_Crazy_Prompt_Generator:
    ...
    def INPUT_TYPES(s):
        return {
            "required": {
                "style_craziness": (
                    "INT",
                    {"default": 1, "min": 1, "max": 5, "step": 1, "display": "number"},
                ),
    ...
    def generate(self, style_craziness, generation_mode):
        ...
        style_counts = {1: 3, 2: 5, 3: 7, 4: 9, 5: 15}
        num_styles = style_counts[style_craziness]

        if generation_mode == "fixed":
            # Use fixed selections
            prompt_parts = {
                category: self.get_fixed_selection(
                    self.categories[category], self.fixed_selections[category]
                )
            ...
            available_styles = [
                s for s in self.categories["styles"] if s not in selected_styles
            ]
            medium_style = (
                self.get_fixed_selection(
                    available_styles, self.fixed_selections["medium_style"]
                )
                if available_styles
                else self.get_fixed_selection(self.categories["styles"], 0)
            )
        else:
            ...
            medium_style = (
                rng.choice(available_styles)
                if available_styles
                else rng.choice(self.categories["styles"])
            )

            # Store selections for fixed mode
            for category in prompt_parts:
                self.fixed_selections[category] = self.categories[category].index(
                    prompt_parts[category]
                )
            self.fixed_selections["styles"] = selected_styles
            self.fixed_selections["medium_style"] = (
                available_styles.index(medium_style) if available_styles else 0
            )
