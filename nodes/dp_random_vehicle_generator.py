...
class DP_Random_Vehicle_Generator:
    def __init__(self):
        ...
        self.camera_angles = self.load_file(os.path.join(data_dir, "vehicle_camera_angle_and_filter.txt"))
        self.color_themes = self.load_file(os.path.join(data_dir, "vehicle_color_theme.txt"))
        self.scene_descriptions = self.load_file(os.path.join(data_dir, "vehicle_scene_description.txt"))
        self.art_styles = self.load_file(os.path.join(data_dir, "vehicle_art_styles.txt"))
    ...
    def generate(self, style_complexity, generation_mode):
        ...
        if generation_mode == "fixed":
            ...
            body = self.get_fixed_selection(self.body_types, self.fixed_selections['body'])
            tires = self.get_fixed_selection(self.tires, self.fixed_selections['tires'])
            camera = self.get_fixed_selection(self.camera_angles, self.fixed_selections['camera'])
            color = self.get_fixed_selection(self.color_themes, self.fixed_selections['color'])
            scene = self.get_fixed_selection(self.scene_descriptions, self.fixed_selections['scene'])
