...
class DP_Image_Color_Effect:
        ...
        return {
            "required": {
                ...
                "color": ([
                    ...
                    "orange", "magenta", "green", "violet", "cyan"
                ],),
                "blend_mode": ([
    ...
    def IS_CHANGED(cls, **kwargs):
        return float("NaN")  # This ensures the node always executes with new random values
    ...
    def apply_blend_mode(self, base, blend, mode, strength):
        if mode == "Normal":
            ...
            return base * (1 - strength) + (
                torch.where(base < 0.5,
                    2 * base * blend,
                    1 - 2 * (1 - base) * (1 - blend))
            ) * strength
        elif mode == "Soft Light":
            return base * (1 - strength) + (
                torch.where(blend < 0.5,
                    base * (2 * blend),
                    1 - (1 - base) * (2 - 2 * blend))
            ) * strength
    ...
    def create_retro_effect(self, image, blend_mode, strength):
        ...
        color_sequence = [[r/255.0, g/255.0, b/255.0] for r, g, b in color_sequence]
        ...
        for color in color_sequence:
            ...
            modified = self.apply_blend_mode(
                image, 
                color_layer, 
                blend_mode,
                strength
    ...
    def process(self, image, color, blend_mode, strength, unique_id):
        try:
            ...
            single_effect = self.apply_blend_mode(image, color_layer, blend_mode, strength)
