...
class DP_Image_And_String_Pairs_Switch:
    ...
    def process(self, cycle_mode, index, unique_id, **kwargs):
        try:
            ...
            connected_indices = []
            for i in range(1, 6):
                if kwargs.get(f"Image_{i:02d}") is not None or kwargs.get(f"String_{i:02d}") is not None:
            ...
            if cycle_mode == "fixed":
                ...
                if index in connected_indices:
                    ...
                    raise ValueError(f"Selected index {index} is not connected. Available indices are: {connected_str}")
            else:
