...
class DP_random_min_max:
    ...
    def INPUT_TYPES(cls):
        return {
            "required": {
                "min": ("FLOAT", {"default": 0.0, "min": -10000.0, "max": 10000.0, "step": 0.1}),
                "max": ("FLOAT", {"default": 1.0, "min": -10000.0, "max": 10000.0, "step": 0.1}),
    ...
    def generate_random(self, min, max, step):
        ...
        if step_value > (max - min):
            raise ValueError(f"Step size {step_value} is too large for range {min} to {max}")
        ...
        try:
            ...
            if step.startswith("float"):
                ...
                random_float_1 = format(round(min + (random.randint(0, steps) * step_value), decimal_places), f'.{decimal_places}f')
                random_float_2 = format(round(min + (random.randint(0, steps) * step_value), decimal_places), f'.{decimal_places}f')
