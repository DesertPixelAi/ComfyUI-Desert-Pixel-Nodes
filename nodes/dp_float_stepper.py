...
class DP_Float_Stepper:
    ...
    def step_value(self, start_point, end_point, step, unique_id=None):
        try:
            ...
            inputs_changed = (
                abs(start_point - self.last_start_point) > 0.0001 or
                abs(end_point - self.last_end_point) > 0.0001 or
                abs(step - self.last_step) > 0.0001
            )
...
# Node registration
NODE_CLASS_MAPPINGS = {
    "DP_Float_Stepper": DP_Float_Stepper
}

NODE_DISPLAY_NAME_MAPPINGS = {
    "DP_Float_Stepper": "Float Stepper"
} 