...
class DP_Advanced_Sampler:
    ...
    def INPUT_TYPES(s):
        return {
            "required": {
                ...
                "overlap_pixels": (
                    "INT",
                    {"default": 0, "min": 0, "max": 128, "step": 8},
                ),
            }
        }
    ...
    def sample(
        ...
        # Initial sampling
        sampled_latent = dp_common_ksampler(
            model,
            seed,
            steps,
            cfg,
        ...
        if enable_upscale:
            ...
            if enable_split:
                ...
                to_upscale["batch_index"] = torch.zeros(
                    len(split_latents), dtype=torch.long
                )
            else:
            ...
            if two_step_upscale:
                ...
                s["samples"] = comfy.utils.common_upscale(
                    to_upscale["samples"], width, height, "bicubic", "disabled"
                )

                # First refinement
                mid_latent = dp_common_ksampler(
                    model,
                    seed + 1,
                    upscale_steps,
                    upscale_cfg,
                ...
                s = mid_latent.copy()
                s["samples"] = comfy.utils.common_upscale(
                    mid_latent["samples"], width, height, "bicubic", "disabled"
                )

                # Final refinement with increased denoise
                upscaled_latent = dp_common_ksampler(
                    model,
                    seed + 2,
                    upscale_steps,
                    upscale_cfg,
