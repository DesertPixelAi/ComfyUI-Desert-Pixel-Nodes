...
class DP_Broken_Token:
    ...
    def INPUT_TYPES(cls):
        return {
            "required": {
                "max_tokens_part_1": (
                    "INT",
                    {"default": 77, "min": 0, "max": 2000, "step": 1},
                ),
                "max_tokens_part_2": (
                    "INT",
    ...
    def analyze_tokens(
        self, max_tokens_part_1: int, max_tokens_part_2: int, prompt: str = None
    ):
        ...
        # Analyze the prompt
        result = FluxTokenizer.analyze_prompt_with_splits(
            prompt, max_tokens_part_1, max_tokens_part_2
        )
