...
class DP_Random_Logo_Style_Generator:
    def __init__(self):
        ...
        self.color_palettes = self.load_file(os.path.join(data_dir, "color_palette.txt"))
    ...
    def generate(self, style_complexity, generation_mode):
        ...
        if generation_mode == "fixed":
            ...
            color = self.get_fixed_selection(self.color_palettes, self.fixed_selections['color'])
            lighting = self.get_fixed_selection(self.lighting, self.fixed_selections['lighting'])
            elements = self.get_fixed_selection(self.additional_elements, self.fixed_selections['elements'])
            ...
            self.fixed_selections['texture'] = self.textures.index(texture[5:])  # Remove "with " prefix
