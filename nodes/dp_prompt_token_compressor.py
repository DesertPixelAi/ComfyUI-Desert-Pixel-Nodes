...
class DP_SmartPromptCompressor:
    ...
    def __init__(self):
        ...
        try:
            self.sd1_tokenizer = AutoTokenizer.from_pretrained(
                "openai/clip-vit-large-patch14"
            )
            self.sd2_tokenizer = AutoTokenizer.from_pretrained(
                "stabilityai/stable-diffusion-2"
            )
            self.sdxl_tokenizer1 = AutoTokenizer.from_pretrained(
                "openai/clip-vit-large-patch14"
            )
            self.sdxl_tokenizer2 = AutoTokenizer.from_pretrained(
                "laion/CLIP-ViT-bigG-14-laion2B-39B-b160k"
            )
            self.flux_tokenizer = AutoTokenizer.from_pretrained(
                "stabilityai/stable-diffusion-xl-base-1.0"
            )  # Flux uses SDXL tokenizer
        except Exception as e:
            print(
                f"Warning: Could not load tokenizers. Using fallback token estimation. Error: {e}"
            )
            self.sd1_tokenizer = self.sd2_tokenizer = self.sdxl_tokenizer1 = (
                self.sdxl_tokenizer2
            ) = self.flux_tokenizer = None
    ...
    def INPUT_TYPES(cls):
        return {
            "required": {
                ...
                "model_type": (
                    ["sd1", "sd2", "sdxl", "flux"],
                    {"default": "sd2", "display_text": "Model Type"},
                ),
                "preservation_mode": (
                    ["balanced", "subject_focus", "style_focus"],
                    {"default": "balanced", "display_text": "Preservation Mode"},
                ),
            },
            "optional": {
                "prompt": (
    ...
    def _compress_prompt(
        self, phrases: List[str], target_tokens: int, mode: str
    ) -> Tuple[str, List[str]]:
        """Compress the prompt by selecting the most important phrases that fit within the token limit."""
        # Score all phrases
        scored_phrases = [
            (phrase, self._score_phrase(phrase, mode)) for phrase in phrases
        ]
    ...
    def compress_prompt(
        ...
        if target_tokens > model_limit:
            print(
                f"Warning: Target tokens ({target_tokens}) exceeds {model_type} limit of {model_limit}. Adjusting..."
            )
        ...
        # Compress prompt
        compressed_prompt, excluded = self._compress_prompt(
            phrases, target_tokens, preservation_mode
        )
