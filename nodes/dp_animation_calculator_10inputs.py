...
class DP_Animation_Calculator_10_Inputs:
    ...
    def INPUT_TYPES(s):
        return {
            ...
            "optional": {**{f"Image_{i:02d}_Input": ("IMAGE",) for i in range(1, 11)}},
        }
    ...
    def calc(
        ...
        try:
            ...
            Image_Start_KeyFrames = [
                kwargs[f"Image_{i:02d}_startPoint"] for i in range(1, 11)
            ]
            ...
            for i, frame in enumerate(Image_Start_KeyFrames):
                if (
                    frame <= total_frames
                    and frame not in frame_dict
                    and images[i] is not None
                ):
                    frame_dict[frame] = i
