...
try:
    ...
    for data in required_nltk_data:
        try:
            nltk.data.find(
                f"tokenizers/{data}" if data == "punkt" else f"corpora/{data}"
            )
        except LookupError:
...
class DP_Prompt_Inverter:
    ...
    def invert_prompt(
        self, inversion_strength, generation_mode, use_nltk=False, input_prompt=None
    ):
        try:
...
# Node registration
NODE_CLASS_MAPPINGS = {"DP_Prompt_Inverter": DP_Prompt_Inverter}

NODE_DISPLAY_NAME_MAPPINGS = {"DP_Prompt_Inverter": "Prompt Inverter"}
