...
class DP_Random_Psychedelic_Punk_Generator:
    ...
    def generate(self, style_complexity, generation_mode):
        ...
        # Add final touch to the prompt
        final_touch = rng.choice(self.final_touch) if generation_mode == "randomize" else self.get_fixed_selection(self.final_touch, self.fixed_selections.get('final_touch', 0))
        
        # Get random or fixed gender
        if generation_mode == "randomize":
            gender = rng.choice(self.gender)
            self.fixed_selections['gender'] = self.gender.index(gender)
        else:
            gender = self.get_fixed_selection(self.gender, self.fixed_selections['gender'])

        # Modify subject to include gender
        subject = subject.replace("a ", f"a {gender} ")

        # Get camera angle
        camera_angle = rng.choice(self.camera_angles) if generation_mode == "randomize" else self.get_fixed_selection(self.camera_angles, self.fixed_selections['camera_angle'])
