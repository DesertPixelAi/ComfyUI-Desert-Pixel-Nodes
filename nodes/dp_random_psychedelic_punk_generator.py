...
class DP_Random_Psychedelic_Punk_Generator:
    ...
    def generate(self, style_complexity, generation_mode):
        ...
        if generation_mode == "fixed":
            ...
            subject = self.get_fixed_selection(
                self.subject, self.fixed_selections["subject"]
            )
            setting = self.get_fixed_selection(
                self.setting, self.fixed_selections["setting"]
            )
            color_palette = self.get_fixed_selection(
                self.color_palette, self.fixed_selections["color_palette"]
            )
            lighting = self.get_fixed_selection(
                self.lighting, self.fixed_selections["lighting"]
            )
            additional_elements = self.get_fixed_selection(
                self.additional_elements, self.fixed_selections["additional_elements"]
            )
            ...
            final_touch = self.get_fixed_selection(
                self.final_touch, self.fixed_selections["final_touch"]
            )
            camera_angle = self.get_fixed_selection(
                self.camera_angles, self.fixed_selections["camera_angle"]
            )
        else:
            ...
            self.fixed_selections["color_palette"] = self.color_palette.index(
                color_palette
            )
            self.fixed_selections["lighting"] = self.lighting.index(lighting)
            self.fixed_selections["additional_elements"] = (
                self.additional_elements.index(additional_elements)
            )
            ...
            self.fixed_selections["camera_angle"] = self.camera_angles.index(
                camera_angle
            )
        ...
        subject = subject.replace(
            "a ", f"a {self.gender[self.fixed_selections['gender']]} "
        )
