...
class DP_FastSlowMotion:
    ...
    def INPUT_TYPES(cls):
        return {
            "required": {
                ...
                "process_all_frames": ("BOOLEAN", {"default": False}),
                "start_frame": (
                    "INT",
                    {
                        "default": 0,
    ...
    def apply_speed_effect(
        self, frames, process_all_frames, start_frame, end_frame, speed
    ):
        try:
            # Ensure frames are in the correct format
            if isinstance(frames, list):
                frames = torch.cat(
                    [self.ensure_tensor_format(f) for f in frames], dim=0
                )
            else:
                frames = self.ensure_tensor_format(frames)

            # If process_all_frames is True, use full frame range
