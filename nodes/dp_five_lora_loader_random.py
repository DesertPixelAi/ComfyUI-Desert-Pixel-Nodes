...
class DP_Five_Lora_Random:
    ...
    def get_random_strength(
        self, base_strength: float, min_val: float, max_val: float
    ) -> float:
        """
    ...
    def apply_lora(
        ...
        loras = [
            (
                Lora_01,
                self.get_random_strength(Lora_01_Strength, Random_Min, Random_Max),
            ),
            (
                Lora_02,
                self.get_random_strength(Lora_02_Strength, Random_Min_2, Random_Max_2),
            ),
            (
                Lora_03,
                self.get_random_strength(Lora_03_Strength, Random_Min_3, Random_Max_3),
            ),
            (
                Lora_04,
                self.get_random_strength(Lora_04_Strength, Random_Min_4, Random_Max_4),
            ),
            (
                Lora_05,
                self.get_random_strength(Lora_05_Strength, Random_Min_5, Random_Max_5),
            ),
        ]
        ...
        if valid_loras:
            for lora_name, strength in valid_loras:
                ...
                is_random = (
                    ...
                    if any(
                        l[0] == lora_name
                        and abs(l[1][1] - l[1][2])
                        > 0.0001  # Check if min != max, accounting for float precision
                        for l in zip(
            ...
            weights_info.append(
                "No LoRAs used"
            )  # New line for when no LoRAs are loaded
