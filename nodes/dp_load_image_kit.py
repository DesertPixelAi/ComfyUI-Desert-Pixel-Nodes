...
class DP_Image_Loader_Medium:
    ...
    RETURN_TYPES = ("STRING", "IMAGE", "IMAGE", "IMAGE", "IMAGE", "STRING", "IMAGE")
    RETURN_NAMES = ("filename", "output1", "output2", "output3", "output4", "prompt", "loaded_image")
    ...
    def load_image_and_process(self, image, output_01_fx, output_02_fx, output_03_fx, output_04_fx, width, height, upscale_method, pipe_input=None):
        ...
        # After loading the image but before applying effects:
        samples = output_image.movedim(-1,1)
        resized = comfy.utils.common_upscale(samples, width, height, upscale_method, "center")
        output_image = resized.movedim(1,-1)
    ...
    def process_outputs(self, output_image, style_map, fx_list):
        outputs = []
        for fx in fx_list:
            if fx in style_map:
                outputs.append(style_map[fx])
            else:
                outputs.append(None)
        return outputs
...
class DP_Image_Loader_Big:
    ...
    def load_image_and_process(self, image, output_01_fx, output_02_fx, output_03_fx, output_04_fx,
                             ...
                             vignette_strength, glow_strength, width, height, upscale_method, pipe_input=None):
        
        # Initialize prompt_text at the start
        prompt_text = ""
        ...
        if pipe_input is not None:
            ...
            try:
                with Image.open(image_path) as img:
                    ...
                    # Try to get prompt from image metadata if not found in caption file
                    if not prompt_text and hasattr(img, 'info'):
                        prompt_text = img.info.get('dp_prompt', '')
...
class DP_Image_Loader_Small(DP_Image_Loader_Medium):
    ...
    RETURN_NAMES = ("output1", "output2", "prompt")
    FUNCTION = "load_image_and_process"
    CATEGORY = "DP/Image"

    @classmethod
    def IS_CHANGED(s, image, output_01_fx, output_02_fx, width, height, upscale_method, pipe_input=None):
        if pipe_input is not None:
            return pipe_input
        image_path = folder_paths.get_annotated_filepath(image)
        return image_path
    ...
    @classmethod
    def VALIDATE_INPUTS(s, image, output_01_fx, output_02_fx, width, height, upscale_method, pipe_input=None):
        if pipe_input is not None:
            return True
        if not folder_paths.exists_annotated_filepath(image):
            return "Invalid image file: {}".format(image)
    ...
    def load_image_and_process(self, image, output_01_fx, output_02_fx, width, height, upscale_method, pipe_input=None):
        prompt_text = ""
...
class DP_Image_Loader_With_Resize(DP_Image_Loader_Medium):
    @classmethod
    def INPUT_TYPES(s):
        input_dir = folder_paths.get_input_directory()
        files = []
        ...
        return {"required": {
            "image": (sorted(files), {"image_upload": True}),
            "mode_setting": ("INT", {"default": 5, "min": 0, "max": 5, "step": 1}),
            "width": ("INT", {"default": 512, "min": 64, "max": 8192, "step": 8}),
            "height": ("INT", {"default": 512, "min": 64, "max": 8192, "step": 8}),
            "upscale_method": (["nearest-exact", "bilinear", "area", "bicubic", "lanczos"], {"default": "lanczos"}),
            "output_01_fx": (available_styles, {"default": "original"}),
    ...
    def load_image_and_process(self, image, mode_setting, width, height, upscale_method, 
                             output_01_fx, output_02_fx, output_03_fx, output_04_fx, pipe_input=None):
        # First load and resize image as normal
        if pipe_input is not None:
            output_image = pipe_input
            formatted_name = "piped_image"
            prompt_text = self.extract_prompt_from_metadata(pipe_input)
        ...
        # Store the resized image before applying effects
        loaded_image = output_image
        ...
        # Process outputs based on mode_setting
        if mode_setting == 0:
            return (formatted_name, None, None, None, None, prompt_text, loaded_image)
        
        outputs = [None] * 4
        
        if mode_setting == 5:  # Process all outputs
            outputs = [
                style_map[output_01_fx],
                style_map[output_02_fx],
                style_map[output_03_fx],
        ...
        elif 1 <= mode_setting <= 4:  # Process only selected output
            fx_map = [output_01_fx, output_02_fx, output_03_fx, output_04_fx]
            outputs[mode_setting - 1] = style_map[fx_map[mode_setting - 1]]

        return (formatted_name, *outputs, prompt_text, loaded_image)
