...
class FluxTokenizer:
    ...
    def _init_tokenizers(cls):
        if cls._tokenizer1 is None:
            try:
                cls._tokenizer1 = AutoTokenizer.from_pretrained(
                    "openai/clip-vit-large-patch14"
                )
                cls._tokenizer2 = AutoTokenizer.from_pretrained(
                    "laion/CLIP-ViT-bigG-14-laion2B-39B-b160k"
                )
            except Exception as e:
                print(
                    f"Warning: Could not load CLIP tokenizers for Flux. Using fallback. Error: {e}"
                )
    ...
    @classmethod
    def split_tokens_multi(
        cls, text: str, max_tokens_1: int, max_tokens_2: int
    ) -> tuple[str, str, str]:
    ...
    @staticmethod
    def analyze_prompt_with_splits(
        text: str, max_tokens_1: int, max_tokens_2: int
    ) -> Dict:
        """Analyze prompt and return token information with three-way split"""
        text = FluxTokenizer.clean_text(text)
        part1, part2, part3 = FluxTokenizer.split_tokens_multi(
            text, max_tokens_1, max_tokens_2
        )
        ...
        return {"info": info, "part1": part1, "part2": part2, "part3": part3}
