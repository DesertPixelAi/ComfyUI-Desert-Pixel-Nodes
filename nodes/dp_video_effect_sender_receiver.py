    ...
    def INPUT_TYPES(cls):
        return {
            "required": {
                ...
                "Effect_key_frame": (
                    "INT",
                    {"default": 24, "min": 1, "max": 10000, "step": 1},
                ),
                "Effect_steps": ("INT", {"default": 3, "min": 3, "max": 10, "step": 1}),
                "Effect_Distance": (
                    "INT",
                    {"default": 1, "min": 1, "max": 3, "step": 1},
                ),
                "Effect_Length": ("INT", {"default": 1, "min": 1, "max": 3, "step": 1}),
            }
        }
    ...
    def execute(
        ...
        if Effect_key_frame >= total_frames:
            return (
                ...
                Load_video_frames[0:0],
                "",
                f"Error: Effect key frame {Effect_key_frame} exceeds video length {total_frames}",
            )
        ...
        effect_frames = (
            torch.stack([Load_video_frames[i] for i in effect_frame_indices])
            if effect_frame_indices
            else Load_video_frames[0:0]
        )  # Empty tensor if no effects
...
class DP_Video_Effect_Receiver:
    ...
    def INPUT_TYPES(cls):
        return {
            "required": {
                ...
                "frames_index": ("STRING", {"multiline": False}),
            }
        }
    ...
    def execute(self, all_frames, processed_frames, frames_index):
        try:
            ...
            try:
                frame_indices = [
                    int(x.strip()) for x in frames_index.split(",") if x.strip()
                ]
            except ValueError:
                return (
                    all_frames,
                    f"Error: Invalid frame indices in '{frames_index}'",
                    all_frames,
                )
            ...
            if max(frame_indices) >= all_frames.shape[0]:
                return (
                    all_frames,
                    f"Error: Frame index {max(frame_indices)} exceeds video length {all_frames.shape[0]}",
                    all_frames,
                )
            ...
            if len(frame_indices) != processed_frames.shape[0]:
                return (
                    all_frames,
                    f"Error: Number of processed frames ({processed_frames.shape[0]}) doesn't match number of frame indices ({len(frame_indices)})",
                    all_frames,
                )
