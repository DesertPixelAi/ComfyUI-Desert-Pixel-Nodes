...
class DPVideoEffectSender:
    ...
    RETURN_NAMES = ("effect_frames", "all_frames", "frames_index", "process_info")
    FUNCTION = "execute"
    ...
    def execute(self, Load_video_frames, frames_to_export,
        ...
        return (effect_frames, all_frames, frames_index, process_info)
    ...
    def execute(self, all_frames, processed_frames, frames_index):
        try:
            ...
            if len(frame_indices) != processed_frames.shape[0]:
                return (all_frames, f"Error: Number of processed frames ({processed_frames.shape[0]}) doesn't match number of frame indices ({len(frame_indices)})")
            ...
            return (output_frames, "Frames replaced successfully")
            
        except ValueError as e:
            return (all_frames, f"Error parsing frame indices: {str(e)}")
        except Exception as e:
            return (all_frames, f"Error processing frames: {str(e)}")
