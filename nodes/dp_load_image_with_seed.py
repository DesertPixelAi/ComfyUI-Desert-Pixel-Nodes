...
class DP_Load_Image_With_Seed:
                        ...
                        if "nodes" in data:
                            for node in data["nodes"]:
                                if "type" in node and "widgets_values" in node:
                                    if any(
                                        sampler in node["type"].lower()
                                        for sampler in [
                                            "sampler",
                                            "dp advanced sampler",
                                        ]
                                    ):
                                        if node["widgets_values"] and isinstance(
                                            node["widgets_values"][0], (int, float)
                                        ):
    ...
    def load_image_and_process(
        self, image, effect_strength, effect_A, effect_B, resize_image, width, height
    ):
        prompt_text = ""
        negative_text = ""
        seed = 0
